...
This Ansible role is designed to install and configure **Docker** to **comply** with the **CIS Docker Benchmark v1.9.0**. \
...
Based on **[CIS Docker Benchmark v1.9.0](https://downloads.cisecurity.org/#/)**.
...
# rule 'cis_docker_rule_1_1_1' is only half implemented, you need to create your storage path/partition manually,
# this rule will only set the configured path with 'cis_docker_data_root', to be used by docker
# if 'cis_docker_rule_2_1' is set, this rule will be skipped
cis_docker_data_root: /var/lib/docker

# if you want run docker in swarm mode, set it to 'true'
# when active: will disable rule 'cis_docker_rule_2_16'
cis_docker_is_swarm_mode: false

# if you want init a docker swarm mode, set it to 'true'
cis_docker_init_swarm_mode: false

# define your own seccomp profile, else default docker one is used
...
cis_docker_rule_5_2: true

cis_docker_rule_7_1: true
...
- name: "Container | Docker installer"
  ...
  roles:
    - role: ansible-docker
      cis_docker_distribution_key_use_modern_sources: true # add apt source in modern style
      cis_docker_user_shell: /bin/bash
      cis_docker_users_to_add_group:
        - name: "{{ ansible_user }}"
      cis_docker_storage_driver: overlay2
      cis_docker_rule_2_1: false # rootless installation mode
      cis_docker_rule_2_10: false # namespace user isolation
      cis_docker_is_swarm_mode: true # enabled as swarm mode
      cis_docker_init_swarm_mode: true # init into swarm mode
      cis_docker_rule_7_5: false
      cis_docker_rule_7_6: false
      cis_docker_rule_7_7: true
      cis_docker_rule_7_8: true
```
...
| :--------------------------------------------------- | :---- |
| 🟢 Implemented                                       | 54    |
| 🟡 Partly Implemented or print info for manual check | 12    |
| 🔴 Not Implemented                                   | 52    |
| Total                                                | 118   |
| Coverage (Implemented/Partly vs Total)               | 52.14 |
...
| 2.4    | Ensure Docker is allowed to make changes to iptables (Automated)                                              | 🟢  |     |     |
| 2.5    | Ensure insecure registries are not used (Automated)                                                           |     | 🟡  |     |
| 2.6    | Ensure aufs storage driver is not used (Automated)                                                            | 🟢  |     |     |
| 2.7    | Ensure devicemapper storage driver is not used (Automated)                                                    | 🟢  |     |     |
| 2.8    | Ensure TLS authentication for Docker daemon is configured (Automated)                                         |     |     | 🔴  |
| 2.9    | Ensure the default ulimit is configured appropriately (Manual)                                                | 🟢  |     |     |
| 2.10   | Enable user namespace support (Automated)                                                                     | 🟢  |     |     |
| 2.11   | Ensure the default cgroup usage has been confirmed (Automated)                                                | 🟢  |     |     |
| 2.12   | Ensure base device size is not changed until needed (Automated)                                               | 🟢  |     |     |
| 2.13   | Ensure that authorization for Docker client commands is enabled (Automated)                                   |     |     | 🔴  |
| 2.14   | Ensure centralized and remote logging is configured (Automated)                                               | 🟢  |     |     |
| 2.15   | Ensure containers are restricted from acquiring new privileges (Automated)                                    | 🟢  |     |     |
| 2.16   | Ensure live restore is enabled (Automated)                                                                    | 🟢  |     |     |
| 2.17   | Ensure Userland Proxy is Disabled (Automated)                                                                 | 🟢  |     |     |
| 2.18   | Ensure that a daemon-wide custom seccomp profile is applied if appropriate (Manual)                           |     | 🟡  |     |
| 2.19   | Ensure that experimental features are not implemented in production (Automated)                               | 🟢  |     |     |
...
| 3.18   | Ensure that daemon.json file permissions are set to 644 or more restrictive (Automated)                       | 🟢  |     |     |
| 3.19   | Ensure that the /etc/default/docker file ownership is set to root:root (Automated)                            | 🟢  |     |     |
...
| 3.22   | Ensure that the /etc/sysconfig/docker file ownership is set to root:root (Automated)                          |     |     | 🔴  |
...
| 5.1    | Ensure swarm mode is not Enabled, if not needed (Automated)                                                   | 🟢  |     |     |
| 5.2    | Ensure that, if applicable, an AppArmor Profile is enabled (Automated)                                        |     | 🟡  |     |
...
| 5.5    | Ensure that privileged containers are not used (Automated)                                                    |     |     | 🔴  |
| 5.6    | Ensure sensitive host system directories are not mounted on containers (Automated)                            |     |     | 🔴  |
| 5.7    | Ensure sshd is not run within containers (Automated)                                                          |     |     | 🔴  |
| 5.8    | Ensure privileged ports are not mapped within containers (Automated)                                          |     |     | 🔴  |
| 5.9    | Ensure that only needed ports are open on the container (Manual)                                              |     |     | 🔴  |
| 5.10   | Ensure that the host's network namespace is not shared (Automated)                                            |     |     | 🔴  |
| 5.11   | Ensure that the memory usage for containers is limited (Automated)                                            |     |     | 🔴  |
| 5.12   | Ensure that CPU priority is set appropriately on containers (Automated)                                       |     |     | 🔴  |
| 5.13   | Ensure that the container's root filesystem is mounted as read only (Automated)                               |     |     | 🔴  |
| 5.14   | Ensure that incoming container traffic is bound to a specific host interface (Automated)                      |     |     | 🔴  |
| 5.15   | Ensure that the 'on-failure' container restart policy is set to '5' (Automated)                               |     |     | 🔴  |
| 5.16   | Ensure that the host's process namespace is not shared (Automated)                                            |     |     | 🔴  |
| 5.17   | Ensure that the host's IPC namespace is not shared (Automated)                                                |     |     | 🔴  |
...
| 5.20   | Ensure mount propagation mode is not set to shared (Automated)                                                |     |     | 🔴  |
| 5.21   | Ensure that the host's UTS namespace is not shared (Automated)                                                |     |     | 🔴  |
| 5.22   | Ensure the default seccomp profile is not Disabled (Automated)                                                |     |     | 🔴  |
| 5.23   | Ensure that docker exec commands are not used with the privileged option (Automated)                          |     |     | 🔴  |
| 5.24   | Ensure that docker exec commands are not used with the user=root option (Manual)                              |     |     | 🔴  |
| 5.25   | Ensure that cgroup usage is confirmed (Automated)                                                             |     |     | 🔴  |
| 5.26   | Ensure that the container is restricted from acquiring additional privileges (Automated)                      |     |     | 🔴  |
| 5.27   | Ensure that container health is checked at runtime (Automated)                                                |     |     | 🔴  |
| 5.28   | Ensure that Docker commands always make use of the latest version of their image (Manual)                     |     |     | 🔴  |
| 5.29   | Ensure that the PIDs cgroup limit is used (Automated)                                                         |     |     | 🔴  |
| 5.30   | Ensure that Docker's default bridge "docker0" is not used (Manual)                                            |     |     | 🔴  |
| 5.31   | Ensure that the host's user namespaces are not shared (Automated)                                             |     |     | 🔴  |
| 5.32   | Ensure that the Docker socket is not mounted inside any containers (Automated)                                |     |     | 🔴  |
| 6      | **Docker Security Operations**                                                                                | 🟢  |     |     |
| 6.1    | Ensure that image sprawl is avoided (Manual)                                                                  | 🟢  |     |     |
| 6.2    | Ensure that container sprawl is avoided (Manual)                                                              | 🟢  |     |     |
| 7      | **Docker Swarm Configuration**                                                                                |     | 🟡  |     |
| 7.1    | Ensure that the minimum number of manager nodes have been created in a swarm (Automated)                      |     | 🟡  |     |
| 7.2    | Ensure that swarm services are bound to a specific host interface (Automated)                                 |     | 🟡  |     |
| 7.3    | Ensure that all Docker swarm overlay networks are encrypted (Automated)                                       |     | 🟡  |     |
| 7.4    | Ensure that Docker's secret management commands are used for managing secrets in a swarm cluster (Manual)     |     | 🟡  |     |
| 7.5    | Ensure that swarm manager is run in auto-lock mode (Automated)                                                | 🟢  |     |     |
